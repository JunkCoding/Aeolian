...
try:
  ...
  pip.main(["install", "heatshrink2"])
  import heatshrink2

try:
  from hiyapyco import odyldo
except:
  pip.main(["install", "hiyapyco"])
  from hiyapyco import odyldo

script_dir = os.path.dirname(os.path.realpath(__file__))

# Need to specify the exxtension on my windows system
npx_cmd = "npx"
if os.name == 'nt':
    npx_cmd = "npx.cmd"
...
def make_pathlist(root):
    ...
    for dir, _, files in os.walk(root, followlinks=True):
        ...
        if reldir and os.path.exists(absdir):
            ...
            entry = (reldir, {'path': absdir, 'type': ESPFS_TYPE_DIR, 'hash': hash})
        ...
        for file in files:
            ...
            if os.path.exists(absfile):
                ...
                entry = (relfile, {'path': absfile, 'type': ESPFS_TYPE_FILE, 'hash': hash})
                bisect.insort(pathlist, entry)
    return pathlist
...
def make_dir_object(hash, path, attributes):
    ...
    header = espfs_object_header_t.pack(ESPFS_TYPE_DIR, espfs_object_header_t.size, index, len(path), 0)
    return header + path
...
def make_file_object(hash, path, data, attributes):
    ...
    for action in actions:
        if action in config['preprocessors']:
            command = npx_cmd + " " + config['preprocessors'][action]['npx_command'][0]
    ...
    if 'gzip' in actions:
        ...
        data = espfs_heatshrink_header_t.pack(window_sz2, lookahead_sz2, 0) + heatshrink2.compress(data, window_sz2 = window_sz2, lookahead_sz2 = lookahead_sz2)
    ...
    header = espfs_object_header_t.pack(ESPFS_TYPE_FILE, espfs_object_header_t.size + espfs_file_header_t.size,
            index, len(path), 0) + espfs_file_header_t.pack(data_len, file_len, flags, compression, 0)
...
def main():
            ...
            if fnmatch(path, pattern):
                ...
                for action in actions[:]:
                    if action == 'cache':
                        ...
                        print('unknown action %s for %s' % (action, pattern), file = sys.stderr)
                        sys.exit(1)
    ...
    offset = espfs_fs_header_t.size + (espfs_hashtable_entry_t.size * num_objects) + (espfs_sorttable_entry_t.size * num_objects)
    hashtable = b''
    ...
    for path, attributes in pathlist:
        ...
        espfs_sorttable_entry_t.pack_into(sorttable, espfs_sorttable_entry_t.size * attributes['index'], offset)
        objects += object
        offset += len(object)

    binary_len = offset + espfs_crc32_footer_t.size
    header = espfs_fs_header_t.pack(ESPFS_MAGIC, espfs_fs_header_t.size,
            ESPFS_VERSION_MAJOR, ESPFS_VERSION_MINOR, binary_len, num_objects, 0)
